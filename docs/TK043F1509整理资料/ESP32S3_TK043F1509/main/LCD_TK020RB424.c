...
#include "esp_log.h"
#include "esp_check.h"
#include "LCD_TK020RB424.h"
#include "driver/gpio.h"
#define TK020RB424_CMD_RAMCTRL               0x00  //no use
#define TK020RB424_DATA_LITTLE_ENDIAN_BIT    (1 << 3)

static const char *TAG = "lcd_panel.TK020RB424";

static esp_err_t panel_TK020RB424_del(esp_lcd_panel_t *panel);
static esp_err_t panel_TK020RB424_reset(esp_lcd_panel_t *panel);
static esp_err_t panel_TK020RB424_init(esp_lcd_panel_t *panel);
static esp_err_t panel_TK020RB424_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end,
                                          const void *color_data);
static esp_err_t panel_TK020RB424_invert_color(esp_lcd_panel_t *panel, bool invert_color_data);
static esp_err_t panel_TK020RB424_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y);
static esp_err_t panel_TK020RB424_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_TK020RB424_set_gap(esp_lcd_panel_t *panel, int x_gap, int y_gap);
static esp_err_t panel_TK020RB424_disp_on_off(esp_lcd_panel_t *panel, bool off);
static esp_err_t panel_TK020RB424_sleep(esp_lcd_panel_t *panel, bool sleep);
...
esp_err_t
esp_lcd_new_panel_TK020RB424(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config,
                         esp_lcd_panel_handle_t *ret_panel)
{
    ...
    TK020RB424_panel_t *TK020RB424 = NULL;
    ESP_GOTO_ON_FALSE(io && panel_dev_config && ret_panel, ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
    TK020RB424 = calloc(1, sizeof(TK020RB424_panel_t));
    ESP_GOTO_ON_FALSE(TK020RB424, ESP_ERR_NO_MEM, err, TAG, "no mem for TK020RB424 panel");
    ...
    case LCD_RGB_ENDIAN_RGB:
        TK020RB424->madctl_val = 0;
        break;
    case LCD_RGB_ENDIAN_BGR:
        TK020RB424->madctl_val |= LCD_CMD_BGR_BIT;
        break;
    default:
    ...
    case 16: // RGB565
        TK020RB424->colmod_val = 0x55;
        fb_bits_per_pixel = 16;
        break;
    case 18: // RGB666
        TK020RB424->colmod_val = 0x66;
    ...
    TK020RB424->ramctl_val_1 = 0x00;
    TK020RB424->ramctl_val_2 = 0xf0;    // Use big endian by default
    if ((panel_dev_config->data_endian) == LCD_RGB_DATA_ENDIAN_LITTLE) {
        // Use little endian
        TK020RB424->ramctl_val_2 |= TK020RB424_DATA_LITTLE_ENDIAN_BIT;
    }

    TK020RB424->io = io;
    TK020RB424->fb_bits_per_pixel = fb_bits_per_pixel;
    TK020RB424->reset_gpio_num = panel_dev_config->reset_gpio_num;
    TK020RB424->reset_level = panel_dev_config->flags.reset_active_high;
    TK020RB424->base.del = panel_TK020RB424_del;
    TK020RB424->base.reset = panel_TK020RB424_reset;
    TK020RB424->base.init = panel_TK020RB424_init;
    TK020RB424->base.draw_bitmap = panel_TK020RB424_draw_bitmap;
    TK020RB424->base.invert_color = panel_TK020RB424_invert_color;
    TK020RB424->base.set_gap = panel_TK020RB424_set_gap;
    TK020RB424->base.mirror = panel_TK020RB424_mirror;
    TK020RB424->base.swap_xy = panel_TK020RB424_swap_xy;
    TK020RB424->base.disp_on_off = panel_TK020RB424_disp_on_off;
    TK020RB424->base.disp_sleep = panel_TK020RB424_sleep;
    *ret_panel = &(TK020RB424->base);
    ESP_LOGD(TAG, "new TK020RB424 panel @%p", TK020RB424);
...
static esp_err_t panel_TK020RB424_del(esp_lcd_panel_t *panel)
{
    TK020RB424_panel_t *TK020RB424 = __containerof(panel, TK020RB424_panel_t, base);

    if (TK020RB424->reset_gpio_num >= 0) {
        gpio_reset_pin(TK020RB424->reset_gpio_num);
    }
    ESP_LOGD(TAG, "del TK020RB424 panel @%p", TK020RB424);
    free(TK020RB424);
    return ESP_OK;
}
...
{
    TK020RB424_panel_t *TK020RB424 = __containerof(panel, TK020RB424_panel_t, base);
    esp_lcd_panel_io_handle_t io = TK020RB424->io;

    // perform hardware reset
    if (TK020RB424->reset_gpio_num >= 0) {
        gpio_set_level(TK020RB424->reset_gpio_num, TK020RB424->reset_level);
                                          ...
                                          const void *color_data)
{
    ...
    x_start += TK020RB424->x_gap;
    x_end += TK020RB424->x_gap;
    y_start += TK020RB424->y_gap;
    y_end += TK020RB424->y_gap;

    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2A00 + 0, (uint16_t[]) {
        (x_start >> 8) & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2A00 + 1, (uint16_t[]) {
        x_start & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2A00 + 2, (uint16_t[]) {
        ((x_end - 1) >> 8) & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2A00 + 3, (uint16_t[]) {
        (x_end - 1) & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2B00 + 0, (uint16_t[]) {
        (y_start >> 8) & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2B00 + 1, (uint16_t[]) {
        y_start & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2B00 + 2, (uint16_t[]) {
        ((y_end - 1) >> 8) & 0xFF,
    }, 2), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0x2B00 + 3, (uint16_t[]) {
        (y_end - 1) & 0xFF,
    }, 2), TAG, "io tx param failed");
    // transfer frame buffer
    size_t len = (x_end - x_start) * (y_end - y_start) * TK020RB424->fb_bits_per_pixel / 8;
...
static esp_err_t panel_TK020RB424_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    esp_lcd_panel_io_handle_t io = TK020RB424->io;
    if (mirror_x) {
        TK020RB424->madctl_val |= LCD_CMD_MX_BIT;
    } else {
        TK020RB424->madctl_val &= ~LCD_CMD_MX_BIT;
    }
    if (mirror_y) {
        TK020RB424->madctl_val |= LCD_CMD_MY_BIT;
    } else {
        TK020RB424->madctl_val &= ~LCD_CMD_MY_BIT;
    }
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]) {
        TK020RB424->madctl_val
...
static esp_err_t panel_TK020RB424_swap_xy(esp_lcd_panel_t *panel, bool swap_axes)
{
    ...
    esp_lcd_panel_io_handle_t io = TK020RB424->io;
    if (swap_axes) {
        TK020RB424->madctl_val |= LCD_CMD_MV_BIT;
    } else {
        TK020RB424->madctl_val &= ~LCD_CMD_MV_BIT;
    }
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]) {
...
{
    TK020RB424_panel_t *TK020RB424 = __containerof(panel, TK020RB424_panel_t, base);
    TK020RB424->x_gap = x_gap;
    TK020RB424->y_gap = y_gap;
    return ESP_OK;
}

static esp_err_t panel_TK020RB424_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    TK020RB424_panel_t *TK020RB424 = __containerof(panel, TK020RB424_panel_t, base);
    esp_lcd_panel_io_handle_t io = TK020RB424->io;
    int command = 0;
