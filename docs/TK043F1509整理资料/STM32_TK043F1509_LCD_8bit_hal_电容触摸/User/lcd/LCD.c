...
* FSMC-D0~D15   : PD 14 15 0 1,PE 7 8 9 10(本屏只用8位数据线即可以)
...
/*   config tft data lines base on FSMC
	   data lines,FSMC-D0~D15: PD 14 15 0 1,PE 7 8 9 10(本屏只初始化8位数据线即可以)
 */	
  
	GPIO_Initure.Pin=GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_14|GPIO_PIN_15; 
...
static void LCD_FSMC_Config(void)       //FSMC 配置函数
{
  ...
  LCD_FSMC.Init.NSBank=FSMC_NORSRAM_BANK1    ;     					        //使用NE1
	...
	LCD_FSMC.Init.MemoryDataWidth=FSMC_NORSRAM_MEM_BUS_WIDTH_8; 	//8位数据宽度
...
{			
	*(__IO u8 *) (Bank1_LCD_C) = CMD>>8;
	*(__IO u8 *) (Bank1_LCD_C) = CMD;
}
static void WriteData(u16 tem_data)       //写数据
{			
	*(__IO u8 *) (Bank1_LCD_D) = tem_data>>8;
	*(__IO u8 *) (Bank1_LCD_D) = tem_data;
}
...
void Lcd_ColorBox(u16 xStart,u16 yStart,u16 xLong,u16 yLong,u16 Color)
{
	...
	{
		*(__IO u8 *) (Bank1_LCD_D) = Color>>8;
		*(__IO u8 *) (Bank1_LCD_D) = Color;
	}
}
/******************************************
函数名：Lcd图像填充
...
void draw_Bat(u16 x,u16 y,u8 power_per)
{
	...
	for(j=0;j<power_per;j++)
	{
		...
		for(i=0;i<6*15;i++)
			{
				*(__IO u8 *) (Bank1_LCD_D) = BLACK>>8;
				*(__IO u8 *) (Bank1_LCD_D) = BLACK;   
			}
		x=x+6;
	}
}
