...
/* Direct I2C device handle — FT6336/FT6206 need STOP between write & read */
static i2c_master_dev_handle_t s_direct_i2c_dev = NULL;
...
esp_err_t esp_lcd_touch_new_i2c_ft5x06(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    /* Init controller — seller confirms FT6336 is fully compatible
     * with FT5x06 register set. Always run init (matches seller's
     * ESP32 example: esp_lcd_touch_CTP_init). */
...
{
    s_direct_i2c_dev = (i2c_master_dev_handle_t)i2c_dev;
}
...
static esp_err_t esp_lcd_touch_ft5x06_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    /* Read touch count from register 0x02 (matches seller's ESP32 example:
     * esp_lcd_touch_CTP_read_data — read CTP_TOUCH_POINTS first, skip gesture). */
    uint8_t pts_byte;
    err = touch_ft5x06_i2c_read(tp, FT5x06_TOUCH_POINTS, &pts_byte, 1);
    if (err != ESP_OK) return err;

    uint8_t points = pts_byte;  /* seller's code uses raw value, not & 0x0F */
    ...
    /* Read all touch data in one burst from register 0x03 (matches seller:
     * touch_CTP_i2c_read(tp, CTP_TOUCH1_XH, data, 6 * points)). */
    uint8_t data[30];
    ...
    /* Parse Y-first (FT6336 layout: P1_Y at 0x03-0x04, P1_X at 0x05-0x06).
     * Matches seller's ESP32 CTP code: Y from data[0..1], X from data[2..3]. */
    for (i = 0; i < points; i++) {
        tp->data.coords[i].x = (((uint16_t)data[i * 6 + 2] & 0x0f) << 8)
                             + data[i * 6 + 3];
        tp->data.coords[i].y = (((uint16_t)data[i * 6 + 0] & 0x0f) << 8)
                             + data[i * 6 + 1];
    }
...
static esp_err_t touch_ft5x06_init(esp_lcd_touch_handle_t tp)
{
    ...
    // Delay to enter 'Monitor' status (s). Set to 0 to stay in Active mode —
    // Monitor mode CLOSES the I2C serial port (per FT5x06 datasheet §2.4),
    // and our board has INT pin disconnected (GPIO_NUM_NC), so we poll via I2C.
    ret |= touch_ft5x06_i2c_write(tp, FT5x06_ID_G_TIME_ENTER_MONITOR, 0);
...
static esp_err_t touch_ft5x06_i2c_read(esp_lcd_touch_handle_t tp, uint8_t reg, uint8_t *data, uint8_t len)
{
    ...
    if (s_direct_i2c_dev) {
        /* FT6336 requires STOP between write and read (matches STM32 reference:
         * FT6206_Read_Reg: START→dev(W)→reg→STOP→START→dev(R)→read N→STOP).
         * Short timeout (5 ms each) — touch timer fires every 10 ms.  If the
         * bus is hung, fail fast instead of blocking LVGL for 100+ ms. */
        esp_err_t ret = i2c_master_transmit(s_direct_i2c_dev, &reg, 1, 5);
        if (ret != ESP_OK) return ret;
        return i2c_master_receive(s_direct_i2c_dev, data, len, 5);
    }

    /* Fallback: combined transaction via esp_lcd_panel_io */
