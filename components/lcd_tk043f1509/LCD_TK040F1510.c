...
ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io,0xBE01,(uint8_t[]) { 0x0055,  }, 2), TAG, "io tx param failed"); // 6A
    ...
    TK040F1510->madctl_val = 0x63;  /* vendor landscape: MV=1 MX=1 MY=0 MH=1 GS=3 */
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io,0x3600,(uint8_t[]) { 0x0063,  }, 2), TAG, "io tx param failed");
    ESP_LOGI(TAG, "MADCTL init: val=0x%02X (MV=%d MX=%d MY=%d BGR=%d)",
             TK040F1510->madctl_val,
             (TK040F1510->madctl_val >> 5) & 1,
             (TK040F1510->madctl_val >> 6) & 1,
             (TK040F1510->madctl_val >> 7) & 1,
             (TK040F1510->madctl_val >> 3) & 1);
    ...
    /* DISP_ON (0x29) moved to lcd_tk043f1509.c — caller fills GRAM black
     * BEFORE turning the display on, so the first visible frame is clean. */
     return ESP_OK;
 }
 ...
 static esp_err_t panel_TK040F1510_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
 {
     ...
     if (invert_color_data) {
         command = LCD_CMD_INVON << 8;
     } else {
         command = LCD_CMD_INVOFF << 8;
     }
 ...
 static esp_err_t panel_TK040F1510_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
 {
     ...
     uint16_t madctl_cmd = ((uint16_t)LCD_CMD_MADCTL << 8) | 0x00;
     ESP_LOGI(TAG, "MADCTL mirror(x=%d,y=%d): 0x%02X",
              mirror_x, mirror_y, TK040F1510->madctl_val);
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, madctl_cmd, (uint8_t[]) {
         TK040F1510->madctl_val
 ...
 static esp_err_t panel_TK040F1510_swap_xy(esp_lcd_panel_t *panel, bool swap_axes)
 {
     ...
     uint16_t madctl_cmd = ((uint16_t)LCD_CMD_MADCTL << 8) | 0x00;
     ESP_LOGI(TAG, "MADCTL swap_xy(%d): 0x%02X", swap_axes, TK040F1510->madctl_val);
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, madctl_cmd, (uint8_t[]) {
         TK040F1510->madctl_val
     }, 1), TAG, "io tx param failed");
     return ESP_OK;
 ...
 static esp_err_t panel_TK040F1510_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
 {
     ...
     int command = 0;
     if (on_off) {
         command = LCD_CMD_DISPON << 8;
     } else {
         command = LCD_CMD_DISPOFF << 8;
     }
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, command, NULL, 0), TAG,
                         "io tx param failed");
     return ESP_OK;
 ...
 static esp_err_t panel_TK040F1510_sleep(esp_lcd_panel_t *panel, bool sleep)
 {
     ...
     int command = 0;
     if (sleep) {
         command = LCD_CMD_SLPIN << 8;
     } else {
         command = LCD_CMD_SLPOUT << 8;
     }
     ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, command, NULL, 0), TAG,
                         "io tx param failed");
